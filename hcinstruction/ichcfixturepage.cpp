...
//    QList<uint> initStatus = onClipToOffClip_.values();
    ...
    initStatus<<ICMold::ACTCLIP1OFF<<ICMold::ACTCLIP2OFF<<ICMold::ACTCLIP3OFF
                <<ICMold::ACTCLIP4OFF<<(ICMold::ACT_AUX3 + 1000)
...
//    currentPosMap_.insert(ui->conveyorCurrentLabel, 7);
    ...
    commandKeyMap_.insert(settingButtons_.at(5), qMakePair(static_cast<int>(IC::VKEY_CLIP5ON), static_cast<int>(IC::VKEY_CLIP5OFF)));
    commandKeyMap_.insert(settingButtons_.at(6), qMakePair(static_cast<int>(IC::VKEY_CLIP6ON), static_cast<int>(IC::VKEY_CLIP6OFF)));
