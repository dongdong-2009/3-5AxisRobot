...
//        painter.drawPixmap(90, 10, icon().pixmap(iconSize()));
        painter.drawPixmap(80, 0, icon().pixmap(iconSize()));
//        painter.drawText(QRect(0, 10, this->width() - 150, iconSize().height()), text(), Qt::AlignRight | Qt::AlignVCenter);
        painter.drawText(QRect(0, 0, this->width() - 150, iconSize().height()), text(), Qt::AlignRight | Qt::AlignVCenter);
        painter.end();
    }
    else
    {
...
//        painter.drawText(QRect(110, 10, this->width(), iconSize().height()), text(), Qt::AlignLeft | Qt::AlignVCenter);
        painter.drawText(QRect(110, 0, this->width(), iconSize().height()), text(), Qt::AlignLeft | Qt::AlignVCenter);
//        painter.drawPixmap(this->rect().right() - iconSize().width() - 130, 10, icon().pixmap(iconSize()));
        painter.drawPixmap(this->rect().right() - iconSize().width() - 110, 0, icon().pixmap(iconSize()));
        painter.end();
    }
